layui.define(['table', 'jquery', 'form'], function (exports) {
    ...
    var MOD_NAME = 'tableMoreSelect',
        $ = layui.jquery,
        table = layui.table,
        form = layui.form;
    var tableMoreSelect = function () {
        this.v = '1.1.0';
    };
    ...
    * 初始化表格选择器
    */
    tableMoreSelect.prototype.render = function (opt) {
        var elem = $(opt.elem);
        ...
        opt.searchPlaceholder2 = opt.searchPlaceholder2 || '关键词搜索';
        ...
        elem.off('click').on('click', function(e) {
            ...
            if($('div.tableMoreSelect').length >= 1){
                return false;
            }

            var t = elem.offset().top + elem.outerHeight()+"px";
            var l = elem.offset().left +"px";
            var tableName = "tableMoreSelect_table_" + new Date().getTime();
            var tableBox = '<div class="tableMoreSelect layui-anim layui-anim-upbit" style="left:'+l+';top:'+t+';border: 1px solid #d2d2d2;background-color: #fff;box-shadow: 0 2px 4px rgba(0,0,0,.12);padding:10px 10px 0 10px;position: absolute;z-index:66666666;margin: 5px 0;border-radius: 2px;min-width:530px;">';
            ...
            var tableMoreSelect_table = table.render(opt.table);
                ...
                if(opt.checkedKey){
                    if(obj.checked){
                        ...
                        if(obj.type=='all'){
                            ...
                            var noCheckedKey = obj.data[opt.checkedKey] || nu2();
                            for (var i=0;i<checkedData.length;i++){
			...
			function updataButton (n) {
				tableBox.find('.tableMoreSelect_btn_select span').html(n==0?'':'('+n+')')
            }
            ...
            //按钮选中
            tableBox.find('.tableMoreSelect_btn_select').on('click', function() {
                var checkStatus = table.checkStatus(tableName);
            ...
            $(document).mouseup(function(e){
                var userSet_con = $(''+opt.elem+',.tableMoreSelect');
    ...
    * 隐藏选择器
    */
    tableMoreSelect.prototype.hide = function (opt) {
        $('.tableMoreSelect').remove();
    }

    //自动完成渲染
    var tableMoreSelect = new tableMoreSelect();

    //FIX 滚动时错位
    if(window.top == window.self){
        $(window).scroll(function () {
            tableMoreSelect.hide();
        });
    }

    exports(MOD_NAME, tableMoreSelect);
})
